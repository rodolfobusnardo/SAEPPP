<?php
session_name('SISTEMA_PERDIDOS_SESSID'); // <-- ALTERAÇÃO FEITA AQUI
session_start();
    ...
    if (in_array(strtolower($username), array_map('strtolower', LOCAL_USERS))) {
        ...
        if ($user = $result->fetch_assoc()) {
            if (password_verify($password, $user['password'])) {
                ...
                // Verificação de Manutenção Pós-Login
                $stmt_maintenance = $conn->prepare("SELECT maintenance_mode FROM settings WHERE config_id = 1");
                $stmt_maintenance->execute();
                $result_maintenance = $stmt_maintenance->get_result();
                $settings = $result_maintenance->fetch_assoc();
                $stmt_maintenance->close();

                if ($settings && $settings['maintenance_mode'] == 1 && $_SESSION['role'] !== 'superAdmin') {
                    // Destrói a sessão recém-criada e redireciona
                    session_unset();
                    session_destroy();
