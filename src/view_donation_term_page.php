...
<div class="container view-term-container admin-container">
    ...
    <div class="term-section">
        ...
        <?php if($term['term_status'] === 'Aprovado' && $term['approved_at']): ?>
            <p><strong>Data da Criação do Termo:</strong> <?php echo date('d/m/Y H:i', strtotime($term['created_at'])); ?></p>
        ...
        <?php elseif($term['term_status'] === 'Doado' && !empty($term['updated_at'])): ?>
            <p><strong>Data da Criação do Termo:</strong> <?php echo date('d/m/Y H:i', strtotime($term['created_at'])); ?></p>
            
            <p><strong>Data da Doação:</strong> <?php echo date('d/m/Y H:i', strtotime($term['updated_at'])); ?></p>
        
        <?php else: // Fallback para outros status (ex: 'Em aprovação') ?>
            <p><strong>Data da Criação do Termo:</strong> <?php echo date('d/m/Y H:i', strtotime($term['created_at'])); ?></p>
        <?php endif; ?>
    </div>
    <div class="term-section">
            ...
            <div class="finalization-card">
                ...
                <form action="finalize_donation_handler.php" method="POST" id="finalizationForm">
                    ...
                    <div class="form-action-buttons-group">
                        ...
                        <button type="submit" id="finalizeDonationButton" class="button-primary">Finalizar Doação</Kbutton>
                    </div>
                </form>
            </div>
        </div>
...
<script>
document.addEventListener('DOMContentLoaded', function() {
    const printDetailedButton = document.getElementById('printDetailedButton');
    const printSummaryButton = document.getElementById('printSummaryButton');
    const detailedItemsSection = document.getElementById('detailedItemsSection');
    const summaryItemsSection = document.getElementById('summaryItemsSection');

    if (printDetailedButton) {
        printDetailedButton.addEventListener('click', function() {
            detailedItemsSection.style.display = 'block';
            summaryItemsSection.style.display = 'none';
            window.print();
        });
    }
    ...
    if (printSummaryButton) {
        printSummaryButton.addEventListener('click', function() {
            detailedItemsSection.style.display = 'none';
            summaryItemsSection.style.display = 'block';
            window.print();
        });
    }
