<?php
// Inclui a verificação de manutenção antes de qualquer outra lógica.
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $current_image_path_2 = null;
    $sql_get_item_details = "SELECT status, image_path, image_path_2 FROM items WHERE id = ?";
    ...
    if ($stmt_get_item_details) {
        ...
        if ($row_details = $result_item_details->fetch_assoc()) {
            ...
            $current_image_path_2 = $row_details['image_path_2'];
        }
    ...
    $new_image_path_1 = $current_image_path;
    $new_image_path_2 = $current_image_path_2;
    $upload_error = null;
    $temp_paths = []; // To store paths of newly uploaded files for potential rollback

    // Helper function for DRY code
    $handle_image_slot = function($image_key, $delete_key, $current_path) use (&$upload_error, &$temp_paths) {
        // Check for new upload
        if (isset($_FILES[$image_key]) && $_FILES[$image_key]['error'] == 0) {
            $processed_path = processUploadedImage($image_key, $upload_error);
            if ($processed_path === false) {
                return false; // Signal failure
            }
            if ($processed_path !== null) {
                // New image successfully processed
                if (!empty($current_path) && file_exists('../' . $current_path)) {
                    unlink('../' . $current_path);
                }
                $temp_paths[] = $processed_path; // Track new file
                return $processed_path;
            }
        }

        // No new upload, check for deletion
        $delete_flag = filter_input(INPUT_POST, $delete_key, FILTER_VALIDATE_INT);
        if ($delete_flag) {
            if (!empty($current_path) && file_exists('../' . $current_path)) {
                unlink('../' . $current_path);
            }
        ...
        return $current_path; // No change
    };

    $new_image_path_1 = $handle_image_slot('item_image_1', 'delete_image_1', $current_image_path);
    if ($new_image_path_1 === false) {
    ...
    $new_image_path_2 = $handle_image_slot('item_image_2', 'delete_image_2', $current_image_path_2);
    if ($new_image_path_2 === false) {
        // Clean up any files that might have been uploaded before the error
        foreach($temp_paths as $path) { if(file_exists('../' . $path)) unlink('../' . $path); }
        $_SESSION['admin_page_error_message'] = $upload_error;
        header('Location: edit_item_page.php?id=' . $item_id);
    ...
    $sql_base_update = "UPDATE items SET name = ?, category_id = ?, location_id = ?, found_date = ?, description = ?, image_path = ?, image_path_2 = ?";
    $types = "siissss"; // Types for the base fields
    $params = [$name, $category_id, $location_id, $found_date, $description, $new_image_path_1, $new_image_path_2];
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            $message = urlencode("Item (ID: " . $item_id . ") atualizado com sucesso!");
            header('Location: /home.php?message_type=success&message=' . $message);
        } else {
            $message = urlencode("Nenhuma alteração foi feita no item (ID: " . $item_id . ").");
            header('Location: /home.php?message_type=success&message=' . $message);
        }
        exit();
    } else {
        error_log("SQL Execute Error (edit_item): " . $stmt->error);
        $message = urlencode("Erro ao atualizar o item no banco de dados.");
        header('Location: /home.php?message_type=error&message=' . $message);
        exit();
    }
    $stmt->close();
