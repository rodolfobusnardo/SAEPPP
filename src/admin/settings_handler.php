...
if ($check_stmt) {
    ...
    if ($check_result->num_rows === 0) {
        ...
        // Ajustado para incluir a coluna maintenance_mode
        $insert_stmt = $conn->prepare("INSERT INTO settings (config_id, unidade_nome, cnpj, endereco_rua, endereco_numero, endereco_bairro, endereco_cidade, endereco_estado, endereco_cep, declaration_donation_text, declaration_devolution_text, maintenance_mode) VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($insert_stmt) {
            // Adicionado 'i' para o inteiro de maintenance_mode
            $insert_stmt->bind_param(
                ...
                $declaration_donation_text, $declaration_devolution_text,
                $maintenance_mode
            );
...
$sql = "UPDATE settings SET
            ...
            endereco_estado = ?,
            endereco_cep = ?,
            declaration_donation_text = ?,
            declaration_devolution_text = ?,
            maintenance_mode = ?
        WHERE config_id = 1";
...
// Bind parameters (11 parameters: ssssssssssi)
$stmt->bind_param(
    "ssssssssssi",
    $unidade_nome,
    $cleaned_cnpj,
