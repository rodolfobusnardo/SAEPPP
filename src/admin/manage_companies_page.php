...
$status_filter = $_GET['status'] ?? 'active'; // Alterado para 'active' como padrão
...
<?php else: ?>
    ...
    <table class="admin-table d-none d-lg-table">
        <colgroup>
        ...
        <tbody>
            <?php foreach ($companies as $company): ?>
                <tr onclick="window.location.href='edit_company_page.php?id=<?php echo $company['id']; ?>';" style="cursor: pointer;">
            ...
            <div class="col-md-6 mb-4">
                <div class="card h-100">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <strong class="text-truncate" style="max-width: 180px;"><?php echo htmlspecialchars($company['name']); ?></strong>
                        <span class="status-badge status-<?php echo htmlspecialchars(strtolower($company['status'])); ?>">
                            <?php echo htmlspecialchars($company['status'] === 'active' ? 'Ativa' : 'Inativa'); ?>
                        </span>
                    </div>
                    <div class="card-body">
                        <p class="card-text mb-2"><strong>CNPJ:</strong> <?php echo htmlspecialchars($company['cnpj'] ?? 'N/A'); ?></p>
                        <p class="card-text mb-2"><strong>Telefone:</strong> <?php echo htmlspecialchars($company['phone'] ?? 'N/A'); ?></p>
                        <p class="card-text mb-2"><strong>Email:</strong> <?php echo htmlspecialchars($company['email'] ?? 'N/A'); ?></p>
                        <p class="card-text"><strong>Responsável:</strong> <?php echo htmlspecialchars($company['responsible_name'] ?? 'N/A'); ?></p>
                    </div>
                    <div class="card-footer">
                        <div class="actions-wrapper-cards">
                            <a href="edit_company_page.php?id=<?php echo $company['id']; ?>" class="button-edit w-100">
                                <i class="fas fa-edit"></i> Editar
                            </a>
                            <form class="w-100" action="delete_company_handler.php" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir esta empresa? Esta ação não pode ser desfeita.');">
                                <input type="hidden" name="company_id" value="<?php echo $company['id']; ?>">
                                <button type="submit" class="button-delete w-100">
                                    <i class="fas fa-trash"></i> Excluir
                                </button>
                            </form>
                        </div>
...
.admin-table .actions-cell .actions-wrapper {
    ...
    justify-content: center;
    align-items: center;
    gap: 5px; /* Define o espaço vertical entre os botões */
}
...
/* ================================================================== */
/* ESTILOS PARA RESPONSIVIDADE (TABLET E MOBILE)              */
/* ================================================================== */
.card-body .card-text {
    font-size: 0.9em;
    margin-bottom: 0.5rem; /* Adiciona um pequeno espaço entre os itens */
}
.card-footer .actions-wrapper-cards {
    display: flex;
    gap: 8px;
}
.card-footer .actions-wrapper-cards > a,
.card-footer .actions-wrapper-cards > form {
    flex: 1; /* Faz os botões dividirem o espaço */
}
.w-100 { /* Classe auxiliar para largura 100% */
    width: 100%;
}

/* ================================================================== */
/* AJUSTE DE TAMANHO DOS BOTÕES DE AÇÃO E FILTRO              */
/* ================================================================== */

/* Botão "Adicionar Nova Empresa" */
.action-button-container .button-primary {
    padding: 4px 10px;
    font-size: 0.85em;
...
.filter-buttons .button-filter-clear {
    padding: 4px 10px;
    font-size: 0.85em;
    min-width: auto; /* Remove a largura mínima para o botão se ajustar ao texto */
    height: auto;
}
